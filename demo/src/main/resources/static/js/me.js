    ...
    function init() {
        ...
        function progressMonitor(evt){
            if (evt.lengthComputable) {
                ...
                var pertime = (nowTime - startTime) / 1000 　　　　　　　　　 // 重新赋值时间，用于下次计算
        ...
        function uploadComplete(evt) {
            ...
            var responses=JSON.parse(evt.currentTarget.response)
            for(var key in responses){
                if (responses[key].code!='200') {
                    alert('上传处理异常' + responses[key].msg)
                    if(responses[key].code=='300')
                        alert("重复")
                    return
                }
            }
